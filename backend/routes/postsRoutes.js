...
//Posting a new post
router.post("", auth, authRequired, multer({ storage: storage }).single("image"), async (req, res, next) => {
	try {
...
router.get("/user/upvoted/:id", auth, async (req, res, next) => {
	try {
		// let lmao = await PostVote.find().populate('post');
		// let delPostVotes = lmao.filter(vote => vote.post === null);
		// let delpostIds = delPostVotes.map(dpv => dpv._id)
		// console.log(delpostIds);

		// for(let i = 0; i < delpostIds.length; i++)
		// {
		//   dpvv = delpostIds[i];
		//   let del = await PostVote.findOneAndDelete({_id: dpvv._id});
		//   console.log(del);
		// }
		...
		if (req.isAuthenticated) {
			...
			if (req.params.id == req.userData.userId) {
				...
				console.log("4");
				let postVotes = await query;
				let userPostVotes = await PostVote.find({ voter: req.userData.userId })
					.populate("post")
					.lean();
			...
			.then(upvotes => {
				if (upvotes) {
					...
					storedRes.forEach(obj => {
						specPost = queryData.find(post => post._id.equals(obj.post));
						if (specPost) {
